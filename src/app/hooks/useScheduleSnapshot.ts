...
import { createDefaultSettings, dedupeSchedulesByKey, ensureDefaultScheduleData } from "../../domain/scheduleSettings";
...
export function useScheduleSnapshot() {
  ...
  function persist(patch: { settings?: ScheduleSettings; votes?: VoteData; result?: GenerateScheduleResult | undefined }) {
    setSavedState("저장 중");
    mergeSnapshot(month, patch);
    window.setTimeout(() => setSavedState("저장됨"), 150);
  }

  function updateSettings(next: ScheduleSettings) {
    ...
    setResult(undefined);
    persist({ settings: sortedSettings, result: undefined });
  }
