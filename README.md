...
- 카카오톡 투표결과 이미지 또는 4열 세로형 CSV 선택 후 자동 입력
- 이미지에서 인식한 미등록 복사일정/차량봉사일정을 일정편집에 자동 추가
- 새 투표 이미지를 선택하면 이전 이미지에서 자동 추가된 일정만 제거한 뒤 새 이미지 일정을 반영
...
이미지 또는 CSV에서 자동 추가된 일정은 출처와 함께 저장되며, 일정편집에서 수정해 저장하면 일반 일정으로 전환됩니다. 이미지 미리보기의 `X`만 누를 때는 일정과 투표결과를 변경하지 않습니다.
투표결과의 복사·차량봉사 섹션을 초기화하면 해당 섹션의 투표 명단과 가져오기에서 자동 추가된 일정만 제거하며, 다른 섹션과 직접 등록하거나 편집한 일정은 유지합니다.

투표 화면의 `카카오톡 투표결과 가져오기`에서 `CSV 업로드` 탭을 열면 샘플 양식을 내려받을 수 있습니다. 샘플의 날짜, 시간, 이름을 실제 투표결과에 맞게 수정한 뒤 `투표결과 CSV 선택`으로 등록합니다.
...
```csv
이름,세례명,축일,정,부,향,향합
홍길동,요셉,03/19,true,true,false,false
```

축일은 `MM/dd` 형식이며 공란으로 둘 수 있습니다. `M/d` 형식도 등록 시 `MM/dd`로 정규화되고, 2월 29일은 허용하지만 존재하지 않는 날짜는 파일 전체를 등록하지 않습니다. 명단 화면은 기본 축일순이며 이름순, 세례명순, 배정 적은순, 배정 많은순으로 정렬 기준을 바꿀 수 있습니다. 세례명과 축일의 공란은 마지막에 표시하고, 같은 값은 이름순으로 정렬합니다. 다운로드 파일은 축일순(공란은 마지막), 같은 축일은 이름순으로 정렬됩니다.

파일 등록 시에는 선택 열 `별칭`을 추가한 `이름,세례명,축일,별칭,정,부,향,향합` 형식도 지원합니다. 기존의 축일 열 없는 CSV도 계속 지원하며, 영문 헤더 `feastDay`도 사용할 수 있습니다. 별칭은 OCR 표시명을 실제 명단 이름으로 연결하기 위한 브라우저 내부 보조 정보이며, 명단 카드에서 추가하거나 수정할 수 있습니다.

명단은 현재 브라우저의 저장소에만 보관되므로 기기 변경이나 브라우저 데이터 삭제에 대비해 명단 화면에서 CSV를 주기적으로 내려받아 백업하는 것을 권장합니다. 명단 화면의 전체 삭제는 저장된 명단만 삭제하며 월별 일정과 투표결과는 유지합니다. 명단과 투표결과 가져오기는 성공 시 반영 건수를 안내하며, 파일 읽기·검증·OCR 처리에 실패하거나 처리 중 기준월·일정·명단이 변경되면 기존 데이터를 유지하고 파일 재선택을 안내합니다.
...
- OCR 입력은 이미지에서 텍스트 행을 분리해 3~4배 확대한 뒤 여러 이진화 강도의 결과를 비교하며, 날짜/시간은 제한된 문자 집합으로 별도 인식합니다.
- 현재 PWA 캐시는 앱 정적 파일 중심입니다. OCR 언어 모델은 배포 파일에 포함되지만 프리캐시하지 않으므로, 최초 온라인 로드 전에는 완전한 오프라인 OCR 동작을 보장하지 않습니다.
