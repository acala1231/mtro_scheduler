...
자동 추가된 OCR 일정은 출처와 함께 저장되며, 일정편집에서 수정해 저장하면 일반 일정으로 전환됩니다. 이미지 미리보기의 `X`만 누를 때는 일정과 투표결과를 변경하지 않습니다.
투표결과의 복사·차량봉사 섹션을 초기화하면 해당 섹션의 투표 명단과 이미지에서 자동 추가된 일정만 제거하며, 다른 섹션과 직접 등록하거나 편집한 일정은 유지합니다.
