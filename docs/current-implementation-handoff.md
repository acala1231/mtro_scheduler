...
- `샘플 양식 다운로드`는 `이름,세례명,정,부,향,향합` 기본 열로 구성된 가짜 명단 CSV 파일을 내려받는다.
...
- 업로드 명단 CSV의 기본 헤더는 `이름,세례명,정,부,향,향합`이다. 파일 등록 시에는 `세례명` 오른쪽에 선택 열 `별칭`을 추가한 `이름,세례명,별칭,정,부,향,향합` 형식도 지원한다.
