...
- 투표결과 이미지를 선택하면 기존 투표결과를 초기화하고 OCR 입력을 즉시 시작한다. 별도 입력/변환 실행 버튼은 없다.
...
- 상단 좌측 뒤로가기 버튼은 홈이 아닌 화면에서 홈으로 이동한다.
- 상단 우측 메뉴는 전체 화면 이동과 명단 접근에 사용한다.
- 하단 네비게이션은 `1 홈 > 2 일정편집 > 3 투표결과 > 4 일정표` 스텝표 형태다.
- 하단 스텝은 숫자, 아이콘, 메뉴명을 함께 표시하고 단계 사이를 화살표로 표현한다.
- 브라우저 물리 뒤로가기/모바일 제스처를 위해 화면 상태는 URL hash와 동기화한다.
- Dialog/Menu/Popover/Snackbar 등 앱에서 제어하는 팝업이 열려 있으면 물리 뒤로가기는 페이지 이동 대신 가장 위에 열린 팝업만 닫는다.
...
- `샘플 양식 다운로드`는 업로드 형식과 같은 가짜 명단 CSV 파일을 내려받는다.
- `명단 다운로드`는 현재 브라우저에 저장된 명단을 CSV로 내려받는다.
- 명단 화면의 `파일로 명단 등록`으로 CSV 파일을 불러온다. 0명일 때는 큰 버튼, 명단이 있을 때는 `...` 파일 메뉴 안에서 제공한다.
- 명단 화면의 `추가` 버튼으로 파일 없이 명단을 한 명씩 수동 추가할 수 있다.
- 업로드 명단 CSV는 `이름,세례명,정,부,향,향합` 헤더를 사용한다.
...
- 세례명은 `baptismalName` 필드로 분리되어 있으며 명단 화면에서 별도 입력 필드로 편집한다.
...
- `관리장님` 문구가 포함된 차량봉사 줄은 표시 인원수와 관계없이 특수 차량봉사 결과로 처리한다.
...
- 사용자 요청은 성격에 따라 `src/app/App.tsx`, `src/app/screens/*`, `src/app/components/*`, `src/app/appUtils.ts`, `src/styles/app.css` 중 적절한 파일에 반영한다.
- `App.tsx`는 화면 조립과 상태 관리 중심으로 유지한다.
...
- OCR 파싱 결과 디버그 모드를 개발자 전용으로 제공
- 미리보기 표 스타일을 테스트 가능한 컴포넌트로 분리
- `localStorage` 저장 구조 버전 마이그레이션 로직 추가
- OCR 정확도 개선을 위한 이미지 전처리 옵션 정리
- `SettingsEditor` 내부 카드 편집 패턴도 추가 공통화할지 검토
- OCR/Tesseract 관련 리소스까지 포함하는 오프라인 캐시 전략 검토
- PWA 설치 안내 UI를 별도로 노출할지 검토
